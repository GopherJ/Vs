 ...
 * @constructor
 */
const GetOffsetTop = (tooltip, target) =>
    offset(target).top - tooltip.node().getBoundingClientRect().height - 10;
 ...
 * @constructor
 */
const GetOffsetLeft = (tooltip, target) =>
    offset(target).left +
    target.getBBox().width / 2 -
...
function showTip(title, el) {
    return function(d) {
        ...
        tooltip.html(isFunction(title) ? title(d) : title);
...
export { showTip, hideTip, isTipShowing };
